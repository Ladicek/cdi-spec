...
For this purpose, primitive types are considered to match their corresponding wrapper types in `java.lang`.
Class types are considered to match if they are identical.
Array types are considered to match only if their component types are identical.
...
A parameterized bean type is considered assignable to a raw required type if the erasure of the bean type is identical to the required type and all type parameters of the bean type are either unbounded type variables or `java.lang.Object`.

A parameterized bean type is considered assignable to a parameterized required type if their erasures are identical and for each type parameter:

* the required type parameter and the bean type parameter are actual types with identical raw type, and, if the required type parameter or the bean type parameter is parameterized, the bean type parameter is assignable to the required type parameter according to these rules, or
...
* the required type parameter is a wildcard, the bean type parameter is a type variable and all upper bounds of the type variable are assignable to or assignable from the upper bound, if any, of the wildcard and assignable from the lower bound, if any, of the wildcard, or
* the required type parameter is an actual type, the bean type parameter is a type variable and the actual type is assignable to all upper bounds of the type variable, or
* the required type parameter and the bean type parameter are both type variables and for each upper bound `B` of the bean type parameter, an upper bound `R` of the required type parameter exists such that `R` is assignable to `B`.
...
A raw bean type is considered assignable to a parameterized required type if the bean type is identical to the erasure of the required type and all type parameters of the required type are either unbounded type variables or `java.lang.Object`.
