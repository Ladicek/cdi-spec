...
* specref:a[A (nonempty) set of bean types]
* specref:b[A (nonempty) set of qualifiers]
* specref:c[A scope]
* specref:e[Optionally, a bean name]
* specref:f[A set of interceptor bindings]
* specref:g[A bean implementation]


Furthermore, specref:h[a bean may or may not be an alternative].
...
* specref:a[A bean type may be an interface], specref:b[a concrete class] or specref:c[an abstract class], and specref:d[may be declared final] or specref:e[have final methods].
* specref:f[A bean type may be a parameterized type with actual type parameters] and specref:g[type variables].
* specref:i[A bean type may be an array type. Two array types are considered identical only if the element type is identical.]
* specref:j[A bean type may be a primitive type. Primitive types are considered to be identical to their corresponding wrapper types in `java.lang`.]
* specref:k[A bean type may be a raw type.]

However, some Java types are not legal bean types :

* specref:la[A type variable is not a legal bean type.]
* specref:lb[A parameterized type that contains a wildcard type parameter is not a legal bean type.]
* specref:lc[An array type whose component type is not a legal bean type.]
