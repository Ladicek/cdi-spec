...
If the set of bean types or qualifiers is infinite, only the `contains()` and `containsAll()` methods may be called on the result of `getTypes()` or `getQualifiers()`; calling the other `Set` methods leads to non-portable behavior.
