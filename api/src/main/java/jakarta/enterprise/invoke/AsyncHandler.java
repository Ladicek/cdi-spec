...
/**
 * Contains logic that determines when an asynchronous action started by an invocation of an asynchronous method
...
public interface AsyncHandler {
     ...
     * in {@code META-INF/services}. An <em>async type</em> of a return type async handler is the erasure
    ...
    interface ParameterType<T> {
         ...
         * transformed such that {@code completion.run()} is called on completion. Note that
         * {@code completion.run()} must be called exactly once. The result is passed to the target method
         * instead of the {@code original}.
         * <p>
         ...
         * returns. When the target method throws an exception synchronously, this method is not called.
         * <p>
         * Takes the {@code original} return value and returns a variant of it, transformed
         * such that {@code completion.run()} is called on completion. Note that {@code completion.run()}
         * must be called exactly once. The result is returned to the invoker caller instead of the {@code original}.
         * <p>
         * It is recommended that {@code completion.run()} is called <em>before</em> completion
         * is propagated to the caller.
         * <p>
